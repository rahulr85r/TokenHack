...
- **2026-08** — Span-staging precision fix + signature quality. `symbol_spans` now drops container-width spans (a matched *class* whose `end_line` covers the whole file) when a tighter span also matched — see `WIDE_SPAN_FRACTION`. Measured over 12 queries on 6 repos, this moved staged-read cost from 68% of whole-file bytes to 16% (a 32% → 83% reduction). Adapters now anchor the stored `signature` on the line containing the symbol's *name* rather than the definition node's first line, so annotated JVM members show `public void channelRead(ChannelHandlerContext ctx, Object msg)` instead of `@Override`. Dead `STAGED_TOKEN_CAP` / `estimate_tokens()` removed; the tunables table now lists all live constants.
